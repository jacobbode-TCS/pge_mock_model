...
from chains.knowledge_search_agent_copy import knowledge_agent
...
def _image_analysis_node(state: WorkflowState) -> WorkflowState:
    ...
    state["analysis"] = classify_bird_image(image_path)
    return state
...
def run_workflow(request: str, image_path: str | None = None, llm_client=None) -> Dict[str, Any]:
    ...
    initial_state = WorkflowState(request=request, image_path=image_path)
